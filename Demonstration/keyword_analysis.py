...
def keywords_with_nltk(cmptr_name, text_corpus, top_n=20):
    ...
    extra_stopwords = ['https', "get", "the", "day", "one", "our", "amp", "like", "use", 'and', 'your', 'you', 'for',
                       ...
                       'more', 'what', 'what', 'out', 'how', 'not', 'here', 'but', 'just', 'co', 'rt', 'indi', 'neat',
                       'got', 'want', 'make', 'new', 'st', 'find', 'slimmingworld', 'ta', 'go', 'innermost',
                       'liveinnermost']
...
def get_keywords():
    ...
    for cmptr in cmptr_list:
        ...
        rps = keywords_with_nltk(cmptr_name=cmptr, text_corpus=' '.join(cmptr_posts), top_n=40)
    ...
    #     # Call the other keyword functions if needed.
    #     rps = keywords_with_nltk(cmptr_name=inflcr, text_corpus=' '.join(inflcr_posts), top_n=40)
    #     final_output.extend(rps)
...
# [2021-05-27] (Marketing) schedule note: Improve influencer clustering around Marketing
